...
import { Shield, AlertTriangle, CheckCircle2, Clock, Zap, Brain, Wrench } from 'lucide-react';
...
function App() {
  ...
  const toUiStatus = useCallback((status: string | undefined | null): Container['status'] => {
  ...
  const handleMessage = useCallback((data: any) => {
    switch (data.type) {
          ...
          setContainers(
            data.containers.map((container: any) => {
              ...
              return {
                ...
                restarts: Number.isFinite(Number(container.restarts)) ? Number(container.restarts) : 0,
                cpu: container.cpu ?? 0,
      ...
      case 'container_update':
        ...
        {
          const timestampIso = typeof data.container.timestamp === 'string'
      ...
      case 'log':
        if (typeof data.message === 'string') {
          const timestampIso = typeof data.timestamp === 'string'
            ? data.timestamp
            : new Date().toISOString();
          const levelRaw = typeof data.level === 'string' ? data.level.toLowerCase() : 'info';
          const level: LogEntry['level'] = ['info', 'warn', 'error', 'debug'].includes(levelRaw)
  ...
  const currentInsight = useMemo(() => {
    ...
    return {
      ...
      suggestedFixes: (analysis.suggested_fixes ?? []).map((fix: any) => ({
        ...
        priority: (fix.priority ?? 1) <= 2 ? 'high' : (fix.priority ?? 1) <= 4 ? 'medium' : 'low',
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-3">
              {incidents.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No incidents detected</p>
              ) : (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-xs px-2 py-1 rounded ${
                        ...
                        incident.status === 'analyzing' ? 'bg-yellow-900/30 text-yellow-400' :
                        'bg-red-900/30 text-red-400'
                      }`}>
                        {incident.status}
                      </span>
                    </div>
                    <p className="text-sm text-gray-400">{incident.service}</p>
        ...
        {/* AI Insights */}
        <AIInsights insight={currentInsight} loading={currentIncident?.analysis == null && currentIncident != null} />

        {/* Log Viewer */}
        <LogViewer
          logs={logs}
          onClear={() => setLogs([])}
          title={primaryContainer ? `${primaryContainer.service ?? primaryContainer.name} Logs` : 'Container Logs'}
        />
      </main>
    </div>
  );
